...
\begin{document}
	...
	\begin{abstract}
		This document outlines the SAR Ship Detection Project, aiming to detect and track ships using Synthetic Aperture Radar (SAR) data from sources like Sentinel-1. 
		The core functionality involves machine learning models for land-sea segmentation and image correction, as well as deep learning models for ship detection and tracking. The system offers three user interfaces: a Jetson AGX Orin for on-orbit inference and data downlink, a web-based interface for on-demand analysis of SAR data selected via a mAP, and an image upload feature for user-provided raw SAR imagery.
		 Key objectives include developing lightweight, high-accuracy models for real-time ship identification, tracking, and potential applications such as monitoring illegal activities, aiding in search and rescue operations, and general maritime surveillance.
		 \\\\Associated Code Repository: https://github.com/ioEclipse/\acrshort{sar}-SHIP-DETECTION 
	...
	To overcome challenges such as speckle noise, sea clutter, and data volume, the system employs machine learning (YOLO-based detection), edge-optimized inference (Jetson/CubeSat), and intelligent AIS cross-matching. The solution aims to provide timely, reliable maritime domain awareness, particularly in high-risk areas like the Central Mediterranean and West African waters, where irregular migration and IUU fishing pose acute humanitarian and security threats. By generating trusted detection, identifying anomalies (e.g., loitering, STS transfers), and enabling real-time operational use, the project sets a foundation for scalable, global maritime surveillance.
	...
	Conventional ship detection methods, often dependent on optical imagery and human observation, are constrained by daylight availability and weather conditions. SAR technology overcomes these limitations by providing all-weather, day-and-night coverage, making it ideal for persistent ocean monitoring.
	...
	\begin{itemize}
		\item Validate detection against known vessel locations.
	...
	Irregular migration and illegal, unreported, and unregulated (IUU) fishing create acute operational demands for maritime domain awareness across the Mediterranean and West African waters, where the central Mediterranean remains the deadliest maritime migration route with at least 3,105 lives lost in 2023 alone. West African fisheries face persistent pressure from industrial fleets and IUU activity, undermining food security and livelihoods in countries such as Senegal, where over exploitation and weak monitoring exacerbate stock depletion and conflict with artisan fleets. These dynamics increase the need for persistent, wide-area, all-weather sensing and timely interdiction support.\\
	...
	This work targets a technical gap: accurate, timely detection and tracking of “dark vessels” (ships that disable or manipulate AIS) by fusing spaceborne SAR detection with AIS data and complementary RF/OSINT where available, optimized for edge execution on resource-constrained platforms (e.g., CubeSats) to enable near-real-time surveillance with reduced downlink requirements.
	...
	\begin{enumerate}
		...
		\item \textbf{Behavioral Event Detection}: Dark activities such as loitering or ship-to-ship transfers require multi-step fusion of AIS trajectories and satellite detection, but current methods suffer from fragile spatiotemporal modeling and limited large-scale validation.
		\item \textbf{Operational Readiness at the Edge}: Few solutions are optimized for CubeSats’ constrained compute, memory, and power budgets, forcing downlink of full SAR scenes instead of prioritized detection, which delays actionable intelligence.
	...
	\begin{itemize}
		\item \textbf{Robust Training Dataset}: Curate and annotate a diverse collection of \acrshort{sar} coverage, including open‑sea, coastal, and congested port scenarios, to achieve at least 10,000 labeled vessel instances by Month 6.
		...
		\item \textbf{Edge Platform Optimization}: Identify the optimal neural‑network architecture for deployment on the NVIDIA Jetson AGX Orin; implement 8‑bit quantization and \acrshort{tensorrt} acceleration to sustain real‑time processing at $\geq 5\cdot frames$ per second by Month 10.
		\item \textbf{Reliable Object Detection \& Tracking}:Integrate detection and multi-object tracking modules to maintain persistent vessel identities across at least 95\% of sequential SAR passes in test scenarios by the twelfth month.
		\item \textbf{\acrshort{ais} Data Fusion}: Correlate \acrshort{sar} detection with \acrshort{ais} broadcasts to confirm $\geq90\%$ of vessel matches, resolve at least 80\% of ambiguous detection, and flag dark‑vessel anomalies for operator review by Month 11.
	...
	\begin{itemize}
		...
		\item Behavioral analytic turn detection into operational leads (loitering, STS), aligning outputs with interdiction workflows rather than raw imagery review.
	...
	It commences with an \textbf{Executive Summary} and \textbf{Introduction}, providing a concise project overview. The \textbf{Project Context and Justification} ground the effort in its broader significance, while the \textbf{Requirements Specification} meticulously lists all functional and non-functional demands. \\
	...
	In 2022, \cite{GablerKoch_DetectionNonCooperativeVessels_2025} Starboard Maritime Intelligence's analysis of Tuvalu's 750,000 km² Exclusive Economic Zone revealed substantial detection discrepancies between SAR and RF technologies. Their study found that RF sensors produced significantly more dark detection than SAR, with many potential false positives attributed to non-AIS transmitting sources including military vessels, yachts, and aircraft radar systems. This highlighted the critical need for multi-sensor validation approaches in dark vessel detection. \\
	...
	\begin{table}[h!]
		...
		\begin{tabularx}{\textwidth}{@{}p{2.7cm}X X X@{}} % narrower first column
			...
			\raggedright Starboard Maritime Intelligence (2022) & RF detections $>$ SAR detection in Tuvalu EEZ & High RF false positives; SAR undercount & FP reduction; SAR+RF fusion accuracy \\
	...
	\begin{longtable}{>{\bfseries}p{4.5cm}p{3.5cm}p{6.5cm}}
		...
		REQ-NFP-ENV-002 & Environment Constraints & The system shall operate within the specified power constraints of Jetson AGX Orin. \\
	...
	\begin{figure}[!ht]
		\centering
		\includegraphics[width=0.55\textwidth]{Lee_filter.png}
		\caption{ \centering Before \& After: Lee filter Application\\ This figure visualizes the effect of applying a Lee filter to a SAR image—a key preprocessing step in ship and dark vessel detection. The left panel (“Original”) shows the raw SAR imagery, where ships and coastal features are visible but surrounded by significant speckle noise. The right panel (“After Lee Filter”) demonstrates how the Lee filter smooths the sea surface, effectively reducing noise while preserving the major coastline and vessel signatures.}
	...
	\begin{figure}[!ht]
		\centering
		\includegraphics[width=0.5\linewidth]{denoising.png}
		\caption{\centering Before \& After: Multi-stage Denoising \newline This figure compares SAR image patches before and after multi-stage denoising and enhancement—a crucial step in the preprocessing pipeline for ship and dark vessel detection. The left image ("Before Enhancement") retains some noise and less pronounced vessel and coastline features. The right image ("After Enhancement") shows clearer separation between land, sea, and detected objects, with reduced noise and more distinct ship-like targets.} 
	...
	\begin{figure}[!ht]
		...
		\begin{subfigure}{0.48\linewidth}
			\centering
			\includegraphics[width=0.9\linewidth]{Morph.png}
			\caption{Before \& After: Morphological Processing}
			\label{fig:morph_comparison}
		\end{subfigure}
		\caption{\centering Comparison of image processing techniques\\ The left subpanel shows the results of Otsu thresholding (global method) and adaptive thresholding (local method). These techniques convert grayscale SAR imagery into binary land–and–sea masks by isolating regions with distinct radar backscatter. Accurate segmentation is fundamental for excluding land from detection processes—ensuring ships and dark vessels in the water are not confused with terrestrial features.\\ The right subpanel illustrates how morphological operations (opening and closing) refine the binary mask produced by thresholding. Before morphology, small gaps and noisy areas remain in the land mask, potentially leading to detection errors. After morphology, the mask is smoothed and small artifacts are removed, resulting in more precise separation between sea and land.			
		}
	...
	\begin{figure}[!ht]
		\centering
		\includegraphics[width=0.3\linewidth]{mask.png}
		\caption{\centering Final Mask Obtained \\ This figure shows the final land mask generated during SAR image preprocessing. The mask delineates land (white) from sea (black), effectively excluding terrestrial regions from subsequent ship detection analysis.}
	...
	\begin{figure}[!ht]
		...
		\caption{\centering Comparization between Original Image and Resuts of first and second preprocessing iteration\\ This figure shows that land masking is applied repeatedly throughout the preprocessing pipeline to fully eliminate land areas from SAR images. The sequence demonstrates how each preprocessing iteration further refines the segmentation mask—progressively removing residual land regions and isolating only the sea.}
	...
	\begin{figure}[!ht]
		...
		\caption{ \centering Before \& After: Noise Reduction \& Edge Enhancement\\
			The figure displays a side-by-side comparison of a SAR (Synthetic Aperture Radar) image before and after noise reduction and edge enhancement. The left image ("Before filter") shows prominent speckle noise and less distinct ship targets, while the right image ("After filter") demonstrates clearer, enhanced ship signatures against a smoother background.}
	...
	Analyzing the graph reveals that YOLOv11n achieves a modest 40\% \acrshort{map} with a latency of 2-3 milliseconds, indicating high speed but insufficient precision for detecting small ships in \acrshort{sar} imagery, where targets often occupy a tiny fraction of the image. \begin{figure}
		...
		\caption{\centering Performance Comparison Graph\\ This performance comparison graph illustrates the trade-off between detection accuracy (COCO mAP@0.5) and inference latency (ms/img) for various YOLO-based models and other architectures on a T4 TensorRT platform. The curve for YOLOv11 demonstrates it achieves high accuracy with low latency, outperforming earlier YOLO versions and alternative models.}
		\label{fig:performance_comparison_graph}
	\end{figure}
	YOLOv11l, while offering a higher \acrshort{map} of 54\% with 8 milliseconds latency, exceeds the computational capacity of the resource-constrained NVIDIA Jetson AGX Orin, a low-power edge device with a 128-core Maxwell GPU, making it impractical for real-time deployment. YOLOv8m and YOLOv10m, with \acrshort{map} values of 46\% and 50\% respectively and latencies around 6 milliseconds, provide moderate performance but fall short in precision for small-object detection and remain borderline for Jetson AGX Orin compatibility.\\
	...
	This section provides a detailed, step-by-step explanation of the data pipeline within the SAR Ship Detection system, elaborating on the high-level architecture presented previously. It provides an overview of the solution implementation and describes the processes that each stage goes through from raw SAR data ingestion to the final output of detected and tracked ships.\\
	
	The data flow within the SAR Ship Detection Project follows a structured pipeline designed for efficiency and accuracy. The primary stages are:
	
	\paragraph{SAR Data Acquisition:}
\begin{itemize}
	\item \textbf{Process}: The pipeline initiates with the acquisition of raw Synthetic Aperture Radar (SAR) imagery. Acquisition of data is done via the Copernicus Browser. For the hardware implementation (Jetson AGX Orin Edge) various raw SAR image files are preloaded into the file system into the Input Folder. The model will inference all the images contained within the folder.  For the cloud implementation the SAR data is either uploaded from the user or queried from Copernicus via user requests initiated through Google Earth.
	\item \textbf{Details:} Data can be downloaded directly from Copernicus in either raw or processed formats. Data can be in multiple file types such as .png and .jpeg as well as .tiff format which allows for use with larger image sizes through special software. The Data is uploaded to the solution via the BlueGuard website which is running the model. Data uploaded by users is first fed into a queue to ensure the page can handle the traffic. Data given to the input folder to the Jetson AGX Orin processes and inferences all of the inputted data. For development purposes it doesn’t delete this data.
\end{itemize}
\paragraph{Preprocessing Queue Management:}
\begin{itemize}
\item  \textbf{Process}: Once a SAR image is ingested, it is placed in a preprocessing queue. This queue serializes and throttles incoming processing requests, ensuring that system memory and CPU/GPU resources are not overloaded when many users access the site concurrently.
\item \textbf{Details}: A simple in-memory First-In-First-Out (FIFO) job queue is implemented using Python. A dedicated background worker thread or asyncio task continuously monitors the queue and processes jobs one by one. While in the queue, users are shown an estimate of their position in the queue and approximate wait time. This is implemented with Streamlit’s session state and periodic UI refresh (st.experimental rerun or st autorefresh).
\end{itemize}

\paragraph{Initial Data Ingestion and Storage:}
\begin{itemize}
	\item \textbf{Process}: Upon receiving a user request via the Streamlit web interface, the system initiates a data ingestion process that either: (1) Downloads SAR imagery from the Copernicus API based on user-specified coordinates and time range. (2) Accepts user-uploaded SAR .tiff, .png, or .jpeg images directly through the web interface. (3) Finally, optionally queries AIS data for the corresponding area and time from a third-party API. Once retrieved or uploaded, the data is temporarily stored for processing and version tracking.
\item \textbf{Details}: Data is stored in local server only while in use. A structured directory or database system is utilized to manage large volumes of SAR images. The images once outputted to the user will be automatically deleted from the server once the unique session is over or times outs. SAR images and associated metadata are stored locally in a structured directory on the host laptop. Each request is isolated in a temporary directory, identified by a unique session or request ID, which avoids filename collisions and supports simple cleanup. Due to limited disk space on the host laptop, data is stored ephemerally. A background cleanup job (using a lightweight Python scheduler like APScheduler) deletes session data older than a set threshold (e.g., 15 minutes or after download).\\

Each ingestion is isolated to its own session. Streamlit’s session-based design supports this naturally, but in addition, image download requests are asynchronous using threads or background tasks to avoid freezing the Streamlit UI.
\end{itemize}
\paragraph{SAR Image Preprocessing (Subsystem):}
\begin{itemize}
	\item \textbf{Process}: The raw SAR images undergo a series of critical transformations to enhance features, reduce noise, and prepare them for machine learning inference. This stage is crucial for improving detection accuracy.
	
	\item \textbf{Details}:
	\begin{itemize}
		
		\item \textbf{Amplitude Scaling and Grayscale Conversion} \\
		\textit{Technique}: Linear intensity amplification and RGB channel duplication. \\
		\textit{Description}: The VV polarization channel is amplified by a factor of 2 to improve contrast between ships and sea clutter, then converted to grayscale by copying it into all three RGB channels.
		
		\item \textbf{Land Masking} \\
		\textit{Techniques}: Lee Filter, CLAHE, Gaussian Blurring, Dual Thresholding, Adaptive Gaussian Thresholding, Morphological Processing, Flood-Fill, Contour Filtering, Iterative Refinement. \\
		\textit{Descriptions}: 
		\begin{itemize}
			\item Lee Filter: Adaptive speckle reduction filter preserves coastline boundaries while smoothing sea regions.
			\item CLAHE: Enhances local contrast within the image while preventing over amplification of noise.
			\item Gaussian Blurring: Reduces high-frequency noise while preserving structure using a Gaussian convolution kernel.
			\item Dual Thresholding: Global thresholding to segment land and sea by minimizing intra-class intensity variance.
			\item Adaptive Gaussian Thresholding: Locally adaptive binarization to handle spatial variations in intensity.
			\item Morphological Processing: Closing and opening operations correct segmentation defects (fill gaps, remove noise).
			\item Flood-Filling: Segments water from land by propagating from known water regions (image corners), then inverts to isolate land.
			\item Contour Filtering: Removes small false-positive land segments by filtering based on area threshold.
			\item Iterative Refinement: Repeats segmentation and filtering until land coverage is within valid thresholds; terminates or refines further based on percentage rules (e.g., $>$85\% land coverage aborts detection).
			
			
			\item \textbf{Noise Reduction and Edge Enhancement} \\
			\textit{Technique}: Gamma filtering and alpha-based enhancement. \\
			\textit{Description}: A gamma correction ($\gamma = 0.6$) suppresses dim background noise while retaining ship features, followed by an enhancement filter ($\alpha = \tfrac{10}{7}$) to increase the brightness of ships and suppress residual noise.
			
		\end{itemize}
		
		\item \textbf{Format Conversion and Standardization} \\
		\textit{Technique}: Conversion to NumPy array and normalization. \\
		\textit{Description}: The preprocessed grayscale image is converted into a normalized NumPy array with pixel values scaled between 0 and 1 for direct input to the deep learning model.
	\end{itemize}
\end{itemize}

\paragraph{Ship Detection Inference (Machine Learning Model):}
\begin{itemize}
	\item \textbf{Process}: The preprocessed SAR image is fed into the trained ship detection deep learning model. This model analyzes the image to identify potential ship instances. The prediction returns the annotated image, various preprocessed images, and a JSON file containing information regarding ship geographical coordinates, confidence scores, etc.
	
	\item \textbf{Details}: The trained model is \texttt{YOLOv11m} for its balance of precision and latency. It was used for preliminary analysis of the overall solution and for development of the end-to-end system. The models are trained via PyTorch with respective datasets (preliminary vs. final), and the weights are converted to ONNX format. These weights can be used for inference in multiple formats, specifically TensorRT, which is deployed on the Jetson AGX Orin architecture. Predictions are limited to ship detection (not classification into ship types). The model is deployed both on the Jetson AGX Orin and on the demo website host.
\end{itemize}

\paragraph{Output Generation:}
\begin{itemize}
	\item \textbf{Process}: The final results, including detected ship bounding boxes, confidence scores, and unique tracking IDs, are compiled and formatted for output. Results are neatly organized into a folder or displayed directly in the web app depending on usage.
	
	\item \textbf{Details}: Output formats include:
	\begin{itemize}
		\item \textbf{Structured Data Output}: Machine-readable files (JSON) containing detection and tracking metadata (coordinates, timestamps, IDs).
		\item \textbf{Annotated Image}: Bounding boxes, labels, and tracking IDs overlaid on the original or preprocessed SAR image. The annotated image can be downloaded as a whole, or individual identified ships can be downloaded separately.
		\item \textbf{Data Downlink/Transmission}: For edge deployments (Jetson AGX Orin), output data is prepared for transmission to ground stations or connected systems. The data is stored in an \texttt{output/} folder once ready.
	\end{itemize}
\end{itemize}

\paragraph{User Interface / Visualization:}
\begin{itemize}
	\item \textbf{Process}: The processed results are delivered to the user through defined interfaces, enabling interaction and visualization of detected and tracked ships. The holistic interface is implemented in Streamlit as a web application containing three sub-interfaces.
	
	\item \textbf{Details}:
	\begin{itemize}
		\item \textbf{Jetson AGX Orin Interface}: Processed data is downlinked and visualized on a local display or streamed to a ground control application, potentially integrating with on-ground AIS data. Statistical insights are also provided. Users cannot request specific data; the Jetson AGX Orin is preloaded with SAR data in the Input Folder. Functionality may be expanded once integrated into a satellite.
		
		\item \textbf{Web-based Map Interface}: Users interact with a Google Earth-style map to select a region of interest (ROI). The ROI is limited to a maximum size and square shape to reduce host machine load. An API fetches the latest SAR data, processes it, and sends it to the model for prediction. Results are then displayed with statistical insights.
		
		\item \textbf{Image Upload Interface}: Users upload raw SAR images through the web interface. The system processes the image, sends it to the model for prediction, and displays the results along with statistical insights.
	\end{itemize}
\end{itemize}
	...
	\subsubsection{Accessing the Jetson AGX Orin}
	To ssh to the Jetson AGX Orin use:
	\begin{lstlisting}
		ssh jetson@<agx_ip_address>
	\end{lstlisting}
	\subsubsection{Essential Command Line Commands}
	\begin{itemize}
		\item \texttt{jtop}: (After \texttt{sudo pip3 install -U jetson-stats} and reboot)  
		A fantastic utility to monitor CPU, GPU, memory, power, and temperatures on your Jetson.  
		Essential for optimizing performance. Type \texttt{q} to quit \texttt{jtop}.
		
		\item \texttt{sudo jetson\_clocks}:  
		Maximizes CPU/GPU/memory clocks for best performance.  
		Useful before running demanding tasks. The \texttt{jtop} tool can also do this.
		
		\item \texttt{sudo reboot}:  
		Restarts the Jetson.
		
		\item \texttt{sudo shutdown now}:  
		Powers off the Jetson.
		
		\item \texttt{yolo export model='your\_model.onnx' format='engine'}:  
		Converts a YOLO model in ONNX format into TensorRT engine format.
	\end{itemize}
 ...
 Following evaluation of all possible matches, the algorithm enriches the detection metadata with AIS information, including positional coordinates, MMSI, timestamp, and the computed distance, time difference, and matching score. The final output is compiled into a structured JSON file, allowing seamless integration into analytical workflows.
  ...
  Critically, if no AIS record satisfies the selection criteria for a given detection, the algorithm classifies the vessel as a “dark ship,” reflecting its absence from global AIS registries and highlighting potential unmonitored maritime activity.\\
	...
	\begin{enumerate}
		...
		\begin{lstlisting}
			!yolo detect train --model yolov11m.yaml --data data/sar_ship.yaml \
			--epochs 20 --img 640 \
	...
	\end{itemize}
	Mathematically, this optimization ensures that $F_{optimized}(x)\approx F_{original}(x)$, where the approximation holds by maintaining the output distribution within an acceptable error margin (typically less than 1\% deviation in mean average precision for vessel detection tasks), while reducing computational overhead by 60–70\% and inference latency by up to four times compared to unoptimized implementations. 
	This optimization is critical for embedded dark vessel detection systems where resource constraints necessitate efficient graph traversal while maintaining the high accuracy required for maritime domain awareness and illegal fishing detection. \\
	...
	This section outlines the end-to-end process for converting the trained \acrshort{yolov11m} model into an optimized inference format suitable for Jetson AGX Orin deployment using \acrshort{tensorrt}. \begin{enumerate}
		...
		\end{lstlisting}
		\item \textbf{Transfer the \acrshort{onnx} Model to Jetson AGX Orin}
		
		Copy the exported \acrshort{onnx} model to the Jetson AGX Orin device using \acrshort{scp} or a USB drive:
		\begin{lstlisting}
		...
		\item \textbf{Convert \acrshort{onnx} to \acrshort{tensorrt} on Jetson AGX Orin}
		
		On the Jetson AGX Orin, use \acrshort{onnx}-\acrshort{tensorrt} or trtexec (included in NVIDIA JetPack) to convert the \acrshort{onnx} model to a \acrshort{tensorrt} engine:
		\begin{lstlisting}
			# Install onnx and tensorrt dependencies (if not already installed)
		...
		\item \textbf{Run Inference on Jetson AGX Orin}
	...
	\begin{figure}[!ht]
		\centering
		\includegraphics[width=0.2\textwidth]{flow_depl.png}
...
\item \textbf{Deployment Target:} NVIDIA Jetson AGX Orin (up to 2048-core Ampere GPU, up to 64GB RAM) to simulate edge-computing constraints
\end{itemize}
	...
	\paragraph{Nadezhda Ivanova} is currently a 11th-grade student studying at the Vocational School for Computer Programming and Innovation in Burgas,Bulgaria. Upon graduation, she will receive both a diploma and the professional qualification of “Artificial Intelligence Programmer.”
	...
	Within the project, Yan applied his programming expertise to collaborate with Anna on refining the preprocessing of SAR data. Their work focused on creating and optimizing the denoising and land segmentation, thereby reducing latency and enhancing the accuracy of the AI model. In addition, Yan contributed to streamlining AIS data flow in the program and supported the integration of the system’s script into the NVIDIA Jetson AGX Orin.
	
	\paragraph{Chahine Med}  is an Engineer and PhD student specialized in computer science, with strong expertise in Artificial Intelligence (AI) and the Internet of Things (IoT). His research and work particularly focus on AI-based simulation and Digital Twin technologies, with skills spanning predictive AI models, computer vision and geospatial applications. 
	
	Within the project, Chahine contributed as an AI and back-end developer. He implemented inference functions based on the trained YOLO model. He also developed APIs and back end services to integrate different components. In addition, he optimized the deep learning model to enhance its performance and efficiency.
